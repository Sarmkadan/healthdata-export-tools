...
namespace HealthDataExportTools.Configuration
{
	...
	public static class CorrelationEngineOptionsExtensions
	{
		...
		/// <exception cref="ArgumentOutOfRangeException">Analysis window is outside valid range.</exception>
		...
		public static bool IsAnalysisWindowValid(this CorrelationEngineOptions options)
		{
			...
			return options.AnalysisWindowDays is >= 7 and <= 365;
		}
		...
		public static int GetEffectiveMaxLagDays(this CorrelationEngineOptions options) =>
			ArgumentNullException.ThrowIfNull(options),
			options.MaxLagDays is var maxLag && maxLag <= 0
				? 0
				: Math.Min(maxLag, options.AnalysisWindowDays / 2);
		...
		public static bool ShouldUseParallelComputation(this CorrelationEngineOptions options) =>
			ArgumentNullException.ThrowIfNull(options),
			options.EnableParallelComputation && options.MaxDegreeOfParallelism > 1;
		...
		/// <param name="options">The correlation engine options.</param>
		/// <exception cref="ArgumentNullException"><paramref name="options"/> is null.</exception>
		/// <exception cref="ArgumentNullException"><paramref name="options.AdditionalMetricPairs"/> is null.</exception>
		...
		public static IReadOnlyList<CorrelationPair> GetComputationPairs(this CorrelationEngineOptions options)
		{
			...
			var pairs = new List<CorrelationPair>(options.AdditionalMetricPairs?.Count ?? 0);

			if (options.AdditionalMetricPairs is not null)
			{
		...
		public static IEnumerable<string> ValidateSignificanceThreshold(this CorrelationEngineOptions options)
		{
			...
			if (options.SignificanceThreshold is <= 0)
			{
				yield return "SignificanceThreshold must be greater than 0.";
			}
			else if (options.SignificanceThreshold > 1.0)
			{
				yield return "SignificanceThreshold should not exceed 1.0 (100%).";
			}

			if (double.IsNaN(options.SignificanceThreshold))
			{
				yield return "SignificanceThreshold must be a valid number.";
			}

			if (double.IsInfinity(options.SignificanceThreshold))
			{
				yield return "SignificanceThreshold must be finite.";
			}
		}
		...
		public static bool ShouldIncludeWeakCorrelations(this CorrelationEngineOptions options) =>
			ArgumentNullException.ThrowIfNull(options),
			options.IncludeWeakCorrelations && options.SignificanceThreshold < 0.3;
	}
}